...
__global__ void fillVBOData(int n, void *vbo, MarkerParticle *particles, GridCellData cellData) {
...
__global__ void raycastPBO(int numParticles, uchar4* pbo, MarkerParticle* particles, Camera camera, GridCellData cellData, LinearNode* tree, int* particleIds) {
...
__global__ void checkParticlesToRender(int* particleIds, MarkerParticle* particles, GridCellData cellData) {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
	...
	/*for (int x = -1; x < 1; ++x) {
		...
		if (inBounds(glm::vec3(0), glm::vec3(GRID_X, GRID_Y, GRID_Z), cellPos + glm::vec3(x, y, z))) {
			if (cellData.cellType[cellId] == AIR) {
...
void raycastPBO(uchar4* pbo, Camera camera) {
    ...
    checkParticlesToRender << <BLOCKS_PARTICLES, BLOCK_SIZE >> > (dev_particleIds, dev_markerParticles, cellData);
    ...
    raycastPBO << <blocksPerGrid2d, BLOCK_SIZE2d >> > (NUM_MARKER_PARTICLES, pbo, dev_markerParticles, camera, cellData, dev_flatTree, dev_particleIds);
    checkCUDAError("raymarch to form PBO failed");
    cudaDeviceSynchronize();
}

__global__ void setAllGridCellsToAir(int n, GridCellData cellData) {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;

    if (index < n) {
		cellData.cellType[index] = AIR;
		cellData.layer[index] = -1;
    }
}

__global__ void setGridCellsWithMarkerParticleToFluid(int n, GridCellData cellData, MarkerParticle* particles) {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;

    if (index < n) {
        MarkerParticle &particle = particles[index];

        const int compressedIndex = getCellCompressedIndex(particle.worldPosition.x / CELL_WIDTH, particle.worldPosition.y / CELL_WIDTH, particle.worldPosition.z / CELL_WIDTH, GRID_X, GRID_Y);

		cellData.cellType[compressedIndex] = FLUID;
		cellData.layer[compressedIndex] = 0;
...
void fillVBOsWithMarkerParticles(void *vbo) {
    ...
    fillVBOData << <blocks, BLOCK_SIZE >> > (NUM_MARKER_PARTICLES, vbo, dev_markerParticles, cellData);
...
__device__ glm::vec3 getInterpolatedVelocity(int cellIndex, glm::vec3 particlePosition, GridCellData cellData) {
	...
	interpolatedVelocity.x = cellData.velocity[cellXMinusIndex].x * (1.0f - xLerp) + cellData.velocity[cellXPlusIndex].x * xLerp;
	interpolatedVelocity.y = cellData.velocity[cellYMinusIndex].y * (1.0f - yLerp) + cellData.velocity[cellYPlusIndex].y * yLerp;
	interpolatedVelocity.z = cellData.velocity[cellZMinusIndex].z * (1.0f - zLerp) + cellData.velocity[cellZPlusIndex].z * zLerp;

	return interpolatedVelocity;
}

__global__ void backwardsParticleTrace(int n, GridCellData cellData) {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (index < n) {
		...
		const glm::vec3 rungeKuttaPosition = cellPosition - (TIME_STEP / 2.0f) * cellData.velocity[index];
		...
		glm::vec3 interpolatedVelocity = getInterpolatedVelocity(getCellCompressedIndex(rungeKuttaPosition.x, rungeKuttaPosition.y, rungeKuttaPosition.z, GRID_X, GRID_Y), rungeKuttaPosition, cellData);
        ...
        cellData.tempVelocity[index] = cellData.velocity[prevCellIndex];
    }
}

__global__ void applyExternalForcesToGridCells(int n, GridCellData cellData) {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;

    if (index < n) {
		if (cellData.cellType[index] == FLUID) {

			// Apply gravity
			cellData.tempVelocity[index] = cellData.velocity[index] + glm::vec3(0, -GRAVITY * TIME_STEP, 0);
		}
		else {
			cellData.tempVelocity[index] = cellData.velocity[index];
		}
    }
}

__global__ void moveMarkerParticlesThroughField(int n, GridCellData cellData, MarkerParticle *particles) {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (index < n) {
		...
		const glm::vec3 interpolatedVelocity = getInterpolatedVelocity(cellIndex, particle.worldPosition, cellData);
		...
		if (rungeKuttaPosition.x < 0 || rungeKuttaPosition.x >= GRID_X || rungeKuttaPosition.y < 0 || rungeKuttaPosition.y >= GRID_Y || rungeKuttaPosition.z < 0 || rungeKuttaPosition.z >= GRID_Z) {
			...
			newVelocity = getInterpolatedVelocity(getCellCompressedIndex(rungeKuttaPosition.x, rungeKuttaPosition.y, rungeKuttaPosition.z, GRID_X, GRID_Y), rungeKuttaPosition, cellData);
		}
...
__global__ void applyViscosity(int n, GridCellData cellData) {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (index < n) {
		...
		if (cellCoords.x + 1 < GRID_X) {
			...
			if (cellData.cellType[adjacent] == FLUID) {
				count++;

				laplacianX += cellData.velocity[adjacent].x;
				laplacianY += cellData.velocity[adjacent].y;
				laplacianZ += cellData.velocity[adjacent].z;
			}
		}
        ...
        laplacianX -= count * cellData.velocity[index].x;
        laplacianY -= count * cellData.velocity[index].y;
        laplacianZ -= count * cellData.velocity[index].z;

		cellData.tempVelocity[index] = cellData.velocity[index] + TIME_STEP * VISCOSITY * glm::vec3(laplacianX, laplacianY, laplacianZ);
    }
}

__global__ void swapCellVelocities(int n, GridCellData cellData) {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;

    if (index < n) {
        cellData.velocity[index] = cellData.tempVelocity[index];
    }
}

__global__ void setupPressureCalc(Grid grid, GridCellData cellData) {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;

    if (index >= grid.numCells)
        return;

	if (cellData.cellType[index] == AIR)
		return;
    ...
    for (int i = 0; i < 6; ++i) {
		...
		if (cellData.cellType[adjacent] == AIR) {
			grid.dev_colIndA[index * 6 + i] = -1;
			//++nonSolid;
			airCells += 1.0f;
			continue;
    ...
    if (cellPos.x + 1 < grid.sizeX) {
        divPlus = 0.5 * (cellData.velocity[index].x + cellData.velocity[index + 1].x);
    }
    if (cellPos.x - 1 >= 0) {
        divMinus = 0.5 * (cellData.velocity[index].x + cellData.velocity[index - 1].x);
    }
    ...
    if (cellPos.y + 1 < grid.sizeY) {
        divPlus = 0.5 * (cellData.velocity[index].y + cellData.velocity[index + grid.sizeX].y);
    }
    if (cellPos.y - 1 >= 0) {
        divMinus = 0.5 * (cellData.velocity[index].y + cellData.velocity[index - grid.sizeX].y);
    }
    divU += (divPlus - divMinus);
    ...
    if (cellPos.z + 1 < grid.sizeZ) {
        divPlus = 0.5 * (cellData.velocity[index].z + cellData.velocity[index + grid.sizeX * grid.sizeY].z);
    }
    if (cellPos.z - 1 >= 0) {
        divMinus = 0.5 * (cellData.velocity[index].z + cellData.velocity[index - grid.sizeX * grid.sizeY].z);
    }
    divU += (divPlus - divMinus);
    divU /= CELL_WIDTH;

    grid.dev_B[index] = divU * (cellData.cellType[index] == FLUID ? FLUID_DENSITY : AIR_DENSITY) / (TIME_STEP) - ATMOSPHERIC_PRESSURE * airCells;
...
__global__ void copyPressureToCells(int numCells, float* pressure, GridCellData cellData) {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index < numCells) {
		cellData.pressure[index] = cellData.cellType[index] == AIR ? ATMOSPHERIC_PRESSURE : pressure[index];
    }
}
...
__global__ void setAirCellPressureAndVelocity(int numCells, GridCellData cellData) {
	const int index = blockIdx.x * blockDim.x + threadIdx.x;
	...
	if (index < numCells) {
		...
		if (cellData.cellType[index] == AIR) {
			...
			if (cellPos.y - 1 >= 0) {
				int belowIndex = getCellCompressedIndex(cellPos.x, cellPos.y - 1, cellPos.z, GRID_X, GRID_Y);
				
				if (cellData.cellType[belowIndex] == FLUID) {
					cellData.pressure[index] = -ATMOSPHERIC_PRESSURE;
...
__global__ void applyPressure(int numCells, GridCellData cellData) {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    if (index < numCells) {
		...
		if (cellData.cellType[index] == FLUID) {
			...
			if (cellPos.x + 1 < GRID_X) {
				deltaPressure.x += cellData.pressure[index + 1];
			}
			if (cellPos.x - 1 >= 0) {
				deltaPressure.x -= cellData.pressure[index - 1];
			}
			if (cellPos.y + 1 < GRID_Y) {
				deltaPressure.y += cellData.pressure[index + GRID_X];
			}
			if (cellPos.y - 1 >= 0) {
				deltaPressure.y -= cellData.pressure[index - GRID_X];
			}
			if (cellPos.z + 1 < GRID_Z) {
				deltaPressure.z += cellData.pressure[index + GRID_X * GRID_Y];
			}
			if (cellPos.z - 1 >= 0) {
				deltaPressure.z -= cellData.pressure[index - GRID_X * GRID_Y];
			}
			...
			cellData.tempVelocity[index] = cellData.velocity[index] - deltaPressure * TIME_STEP / ((cellData.cellType[index] == FLUID ? FLUID_DENSITY : AIR_DENSITY) * CELL_WIDTH);
		}
	}
}

__global__ void setVelocitiesIntoSolidsAsZero(int numCells, GridCellData cellData) {
	const int index = blockIdx.x * blockDim.x + threadIdx.x;

	if (index < numCells) {
		const glm::vec3 cellPos = getCellUncompressedCoordinates(index, GRID_X, GRID_Y);
		glm::vec3& velocity = cellData.velocity[index];

		if (cellPos.x == GRID_X - 1 && velocity.x > 0) {
			//cell.velocity.x *= -1.0;
			velocity.x = 0.0f;
		}
		if (cellPos.x == 0 && velocity.x < 0) {
			//cell.velocity.x *= 1.0;
			velocity.x = 0.0f;
		}

		if (cellPos.y == GRID_Y - 1 && velocity.y > 0) {
			//cell.velocity.y *= -1.0;
			velocity.y = 0.0f;
		}
		if (cellPos.y == 0 && velocity.y < 0) {
			//cell.velocity.y *= 1.0;
			velocity.y = 0.0f;
		}

		if (cellPos.z == GRID_Z - 1 && velocity.z > 0) {
			//cell.velocity.z *= -1.0;
			velocity.z = 0.0f;
		}
		if (cellPos.z == 0 && velocity.z < 0) {
			//cell.velocity.z *= 1.0;
			velocity.z = 0.0f;
		}
	}
...
__global__ void extrapolateFluidVelocities(int numCells, GridCellData cellData, int layer)
{
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
	...
	if (index < numCells) {
		...
		if (cellData.layer[index] == -1) {
			for (int d = 0; d < 6; ++d) {
				...
				if (cellPos.x + x >= 0 && cellPos.x + x < GRID_X && cellPos.y + y >= 0 && cellPos.y + y < GRID_Y &&cellPos.z + z >= 0 && cellPos.z + z < GRID_Z) {
					int adjacent = getCellCompressedIndex(cellPos.x + x, cellPos.y + y, cellPos.z + z, GRID_X, GRID_Y);

					if (cellData.layer[adjacent] == layer - 1) {
						//if (cells[adjacent].cellType != FLUID) {
							averageVelocity += cellData.velocity[adjacent];
							count++;
						//}
					}
				}
			...
			cellData.layer[index] = layer;
			cellData.tempVelocity[index] = (averageVelocity + cellData.velocity[index]) / (float(count) + 1.0f);
		} else {
			cellData.tempVelocity[index] = cellData.velocity[index];
		}
	}
...
__global__ void resetGridCells(int numCells, GridCellData cellData) {
	const int index = blockIdx.x * blockDim.x + threadIdx.x;

	if (index < numCells) {
		cellData.velocity[index] = glm::vec3(0);
		cellData.tempVelocity[index] = glm::vec3(0);
	}
}

void initPressureGrids() {
...
void initSim() {
    ...
    cudaMalloc(&cellData.layer, NUM_CELLS * sizeof(int));
	cudaMalloc(&cellData.cellType, NUM_CELLS * sizeof(CellType));
	cudaMalloc(&cellData.pressure, NUM_CELLS * sizeof(float));
	cudaMalloc(&cellData.velocity, NUM_CELLS * sizeof(glm::vec3));
	cudaMalloc(&cellData.tempVelocity, NUM_CELLS * sizeof(glm::vec3));
    checkCUDAError("malloc grid cells failed");

    // Init pressure grids
    initPressureGrids();
    checkCUDAError("init pressure grids failed");
...
void restartSim() {
	...
	resetGridCells << <BLOCKS_CELLS, BLOCK_SIZE >> > (NUM_CELLS, cellData);
...
void iterateSim() {
    ...
    setAllGridCellsToAir << <BLOCKS_CELLS, BLOCK_SIZE >> > (NUM_CELLS, cellData);
    checkCUDAError("marking all cells as air cells failed");
    cudaDeviceSynchronize();

    // Mark all cells with a marker particle as a fluid cell
    setGridCellsWithMarkerParticleToFluid << <BLOCKS_PARTICLES, BLOCK_SIZE >> > (NUM_MARKER_PARTICLES, cellData, dev_markerParticles);
    ...
    backwardsParticleTrace<<<BLOCKS_CELLS, BLOCK_SIZE>>>(NUM_CELLS, cellData);
    ...
    swapCellVelocities << <BLOCKS_CELLS, BLOCK_SIZE >> > (NUM_CELLS, cellData);
    checkCUDAError("swapping velocities in cells failed");
    cudaDeviceSynchronize();

    // Apply external forces to grid cell velocities
    applyExternalForcesToGridCells << <BLOCKS_CELLS, BLOCK_SIZE >> > (NUM_CELLS, cellData);
    ...
    applyViscosity<<<BLOCKS_CELLS, BLOCK_SIZE>>>(NUM_CELLS, cellData);
    ...
    setupPressureCalc << <BLOCKS_CELLS, BLOCK_SIZE >> > (grids[0], cellData);
    ...
    copyPressureToCells << <BLOCKS_CELLS, BLOCK_SIZE >> > (NUM_CELLS, grids[0].dev_X, cellData);
    checkCUDAError("copy pressure to cells failed");
    cudaDeviceSynchronize();

    // Apply pressure
    applyPressure << <BLOCKS_CELLS, BLOCK_SIZE >> > (NUM_CELLS, cellData);
	...
	for (int i = 1; i < 5; i++) {
		extrapolateFluidVelocities << <BLOCKS_CELLS, BLOCK_SIZE >> > (NUM_CELLS, cellData, i);
	...
	setVelocitiesIntoSolidsAsZero << <BLOCKS_CELLS, BLOCK_SIZE >> > (NUM_CELLS, cellData);
	checkCUDAError("setting velocities into solids as zero failed");
	cudaDeviceSynchronize();

    // Move the marker particles through the velocity field
    moveMarkerParticlesThroughField << <BLOCKS_PARTICLES, BLOCK_SIZE >> > (NUM_MARKER_PARTICLES, cellData, dev_markerParticles);
